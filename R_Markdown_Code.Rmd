...
result1 <- lm(price ~ . - date - id- sqft_basement - good_quality-  house_size, data = train)
summary(result1)
...
```{r}
result <- lm(price ~ . - date - sqft_living - id - good_quality-  house_size, data = train)
summary(result)
```

```{r}
train_automated_df = train[,-c(1:2,22:23)]
...
```{r}
logdata <- train[,-c(1:2,22:23)]
logdata$y.star<-log(logdata$price)
result.ystar<-lm(y.star~.- sqft_living, data=logdata)
par(mfrow = c(2, 2))
plot(result.ystar)
```
